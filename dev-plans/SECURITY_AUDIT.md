...
**Last refreshed:** 2026-07-28 against 1.7.3.
...
> **Refresh note (2026-07-28, 1.7.3):** No F-* status flips this cycle — 1.7.3 shipped no security-tier workitems (SC/SA/AU). It is a patch release: two ESPHome 2026.7 toolchain bug fixes, a 14-PR dependency sweep, and two build-infrastructure fixes. Four security-relevant deltas worth recording:
>
> - **Both open high-severity Dependabot alerts cleared** — `brace-expansion` (< 5.0.7) and `fast-uri` (< 3.1.4), each flagged against `ha-addon/ui/package-lock.json`. Both are **dev-only transitives** and ship in nothing users run: `brace-expansion` arrives via `eslint → minimatch`, `fast-uri` via `shadcn → @modelcontextprotocol/sdk → ajv`. Neither is in the served bundle's request-handling surface. Bumped to 5.0.7 / 3.1.4; `npm audit --audit-level=high --omit=dev` is clean.
> - **Two moderate production findings remain, and are blocked on a held-back upgrade.** `npm audit --omit=dev` reports nine DOMPurify advisories reached through `monaco-editor` 0.55.1, with `fixAvailable: {monaco-editor 0.56.0, isSemVerMajor: true}` as the only remedy. That bump (PR #221) breaks the TypeScript build — moved module paths plus `inlineSuggest.enabled` changing from boolean to a string union — so it was deliberately held out of the release and tracked as **DEP.2** in `WORKITEMS-1.8.md`. **Assessed as not exploitable in this configuration, verified rather than assumed:** every one of the nine advisories lives in DOMPurify's markdown/HTML sanitisation path, and Fleet never reaches it. `src/components/editor/completionProvider.ts` types `ConfigVar.docs` as a plain `string`, which monaco renders as text rather than as an `IMarkdownString`, and the app registers **no hover provider at all** (`registerHoverProvider` appears nowhere in `src/`). Worth noting the content in question *is* third-party — `src/api/esphomeSchema.ts` fetches from `schema.esphome.io`, the one file in `api/` that talks to a host we don't run — which is precisely why the plain-string typing matters. The assessment holds only while that stays true: passing an `IMarkdownString` into the editor would open the path, so DEP.2 should land rather than rot.
> - **Python dependency floors raised across the server and worker.** `aiohttp` 3.14.3, `aioesphomeapi` 45.7.0, `apscheduler` 3.11.3, `croniter` 6.2.4, `idna` 3.18 (both images). `requirements.lock` regenerated through `scripts/refresh-deps.sh` (linux/amd64 in `python:3.13-slim`) with `--generate-hashes`; PY-8 and PY-9 clean, no macOS-only transitives leaked. The client lock came out byte-identical, so the worker image content is unchanged and no `IMAGE_VERSION` / `MIN_IMAGE_VERSION` bump was warranted — deployed workers are not forced to rebuild.
> - **Lint toolchain pinned (#245), which cuts both ways.** `ci.yml` installed `ruff` and `mypy` unpinned, so the gate consumed whatever was newest on PyPI at job time — an unreviewed third-party binary on every run, and in practice a gate that went red on unmodified code the day ruff 0.16.0 shipped. Now pinned to `ruff==0.15.22` / `mypy==2.3.0`. Supply-chain posture improves (the analysis tooling is reproducible and reviewable); the tradeoff is that pins now need deliberate bumping, tracked as **LNT.1**. No product code path is involved either way.
>
